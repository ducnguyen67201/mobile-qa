...
pub async fn claim(
    ...
    let id = if let Some(r) = prior.first() {
        ...
        let jobs = rows(
            ...
            r.manifest->'profile'->>'id'=$2 AND ($4 OR NOT (r.manifest ? 'source')) AND ($3 OR NOT jsonb_path_exists(r.manifest, '$.cases[*].case.actions[*] ? (@.kind == \"direct\")')) AND (r.manifest->'resolved_model' IS NULL OR r.manifest->'resolved_model'='null'::jsonb OR EXISTS (SELECT 1 FROM jsonb_array_elements($5::jsonb) compatible WHERE compatible->'reference'=r.manifest->'resolved_model'->'reference' AND compatible->'provider'=r.manifest->'resolved_model'->'provider')) ORDER BY r.created_at,r.id,CASE WHEN r.manifest->'source'->>'kind'='saved_suite_v1' THEN 0 ELSE a.case_index END,a.number,a.id FOR \
