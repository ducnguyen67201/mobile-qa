...
//! inherited production DATABASE_URL. The connection check uses SeaORM directly.
...
async fn registered_handler_agrees_with_openapi_and_database() {
    ...
    let expected: std::collections::BTreeSet<_> = browser::OPERATIONS
        ...
        .chain(mobile_qa_contracts::artifacts_api::OPERATIONS)
        .chain(mobile_qa_contracts::device_hosts_api::OPERATIONS)
        .map(|(_, path, _, _)| *path)
        .collect();
    assert_eq!(
        paths
    ...
    request::<App, _, _>(|request, ctx| async move {
        ...
        assert_eq!(
            wrong_method.json::<ApiError>().request_id.to_string(),
            wrong_method.header("x-request-id")
        );
        ...
        assert!(!database_url.path().trim_start_matches('/').is_empty());
    })
    .await;
}
