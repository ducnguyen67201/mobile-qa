...
export function TaskSession() {
  ...
  return <Phone key={`${workspaceId}:${app_id}`} appId={app_id} />
}
function Phone({ appId }: { appId: string }) {
  ...
  useEffect(() => {
    ...
    if (!choices.data.blockers.length && choices.data.profiles.length === 1) {
      initiated.current = true
      createSession({ id: requestId, build_id: null, profile_id: null })
    }
  }, [choices.data, createSession, requestId])
    ...
    <Stack gap="lg">
      <Group justify="space-between">
        <div>
          ...
          <Text c="dimmed">Describe a task. Watch Minitap do it.</Text>
        </div>
      ...
      <div style={{ display: 'flex', gap: 32, flexWrap: 'wrap', alignItems: 'flex-start' }}>
        <Card withBorder radius="xl" p="sm" style={{ width: 320, maxWidth: '100%', flexShrink: 0 }}>
          {frame ? (
                ...
                frame.controls.map((c) => (
                  <button
                    ...
                    style={{
                      ...
                      border: selected?.id === c.id ? '2px solid #245b46' : '1px solid transparent',
                    }}
                  />
                ))}
            </div>
            ...
            <Stack align="center" justify="center" style={{ minHeight: 430 }}>
              {!done && !choices.data?.blockers.length && <Loader />}
              <Text ta="center">
                {s?.message ??
        ...
        <Stack style={{ flex: '1 1 340px', minWidth: 0 }}>
          {s && (
            <Group>
              <Badge>{s.state.replaceAll('_', ' ')}</Badge>
