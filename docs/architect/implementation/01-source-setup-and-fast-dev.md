...
3. Describe `/api/health` in the Rust contract, bind it to the real handler, and consume its generated client from `apps/web`. Preserve React Router and TanStack Query. The original foundation used one shadcn layout; spec 03 supersedes it with packaged Mantine controls.
