...
## 27. Proposed commercial access and usage authorization

The fixed per-check recurring terms below describe legacy local agreements and routes. Section 28 defines the successor for new paid subscriptions.

The first commercial interface supports an operated service, not self-service payment. A trusted internal process activates an accepted agreement for one qualified Android app and one qualified device profile; a customer workspace operator cannot grant access or alter a price. An uncontracted app can retain setup, saved work, and read access but cannot start device/model work. Its authenticated user may record one app-scoped pilot request with a coverage note for manual operator review; this does not accept an offer or start an invoice. Expiry or pause stops new work without hiding existing reports. Hosted activation remains contingent on the pilot technical gates above.

The proposed founding pilot is US$500 once for a 14-day period and four included reviewed release checks within Section 20's scope. The proposed recurring offer uses a US$250 base per explicitly agreed period for one suite of at most ten cases and up to one hour of minor maintenance, plus US$125 per authorized reviewed check. A second agreed suite of at most ten cases and one additional maintenance hour adds US$250 per period. The initial recurring cap is eight checks per period. Prices, demand, support effort, taxes and renewal intent are unvalidated; no period renews or prorates automatically.

One billable check means one agreed saved suite or release plan on one accepted build and qualified profile, within the proposed 30-minute full-run bound. Comparing two builds consumes two checks; running two suites on one build consumes two. Each check has a server-generated, short-lived quote for its exact build, saved version, device and environment. The customer must authorize the displayed amount before the run is accepted. An accepted run reserves one check atomically; an idempotent retry cannot reserve another. Pilot inclusions show US$0 additional charge. No automatic overage or unlimited use is offered. Standalone saved-case runs, interactive phone actions and AI generation require a separate operator-scoped agreement and cannot be used to bypass the check cap.

The run verdict does not determine billing. A reviewed, usable report documenting an app failure is a delivered check. If our infrastructure prevents a usable report, the operator reruns or credits it with a reason; a queued run canceled before device claim is credited. Unresolved work is kept separate from delivered charges. The customer sees base/add-on, reserved, delivered and credited checks and the matching run IDs. Invoicing and payment stay manual until cost and reliability are measured and a separate billing integration is designed. The agreed contract controls support, taxes, cancellation and payment terms.

The legacy pilot request, operator agreement and check ledger remain in the API for existing contracts. The customer Settings page now shows the Section 28 monthly plans and credit usage. An exact quote and explicit authorization still appear on the run screen before any reservation.

## 28. Monthly credit plans — local implementation

The new offer is a monthly allowance for one Android app. Unused credits expire at each paid subscription period's end; they do not roll over and are not cash. It replaces the proposed per-check and second-suite charges for newly paid subscriptions. Existing manual agreements remain separate legacy records. Prices and rate-card margins remain unvalidated against real customer-app costs.

Present three monthly plan cards, with Plus visually recommended and the same initial price per credit across tiers:

| Plan     | Monthly price | Included credits | Scope                                                                          |
| -------- | ------------: | ---------------: | ------------------------------------------------------------------------------ |
| Starter  |        US$500 |           50,000 | One Android app; qualified saved suites and release plans share the allowance. |
| Plus     |        US$750 |           75,000 | The same app scope with more monthly usage.                                    |
| Business |      US$1,500 |          150,000 | The same app scope with a larger monthly allowance.                            |

The paywall shows the price, included allowance, reset rule and current rate card. One action on each card creates a Stripe-hosted subscription checkout for a new app; there is no coverage questionnaire. An uncertain checkout response retains the app's intent and Stripe idempotency key for retry. An older intent frees the app only after Stripe confirms the session expired; an intent without a known session needs billing review. An existing subscriber may schedule one different plan at the next renewal. The API uses a Stripe subscription schedule with the current price until the paid period ends, no proration, and a future one-month phase that releases the subscription on completion at the new price. The future price has a product named for the selected plan so the renewal invoice is labeled correctly. A pending change can be canceled to keep the current plan, then another plan may be selected. The UI shows both the current allowance and the pending plan/date. No new checkout or credit grant is created by scheduling. The API only creates a period grant after a signed `invoice.paid` webhook, with the subscription, invoice period, paid amount and expected plan price checked against Stripe. A valid paid renewal changes the granted allowance and clears the pending state once; replayed invoices cannot duplicate grants. Failed payment does not change the allowance. Returning from Checkout does not itself grant credits. Production needs Stripe secret and webhook credentials injected into the API process, plus a registered webhook endpoint. There are no credentials in the local repository.

The initial rate card, revision 1, charges 200 credits per million provider input tokens, 800 per million output tokens, 60 per device minute, and 100 per GiB of sealed evidence. These are provisional prices. A usable reviewed report can consume credits even when it records an app failure. Infrastructure failure that prevents a usable report releases the reservation after operator review. Missing provider token counts or device timestamps block settlement; the operator must resolve the meter, rather than silently treating it as zero.

Calculate with integer units and round up once per run: `credits = ceil(input_tokens × input_rate_per_million / 1,000,000 + output_tokens × output_rate_per_million / 1,000,000 + device_seconds × device_rate_per_minute / 60 + stored_bytes × evidence_rate_per_GiB / 1,073,741,824)`. The evidence rate covers the agreed retention window; longer storage needs a separate scope. The quote freezes the rate-card revision and a maximum spend before authorization. Credit rates are customer prices, not raw provider pass-through costs.

The quote reserves a maximum of 3,000 token credits plus the manifest's device-time and evidence budgets at the published rates. Its rate revision and run identity are frozen for ten minutes. The run-creation transaction atomically holds that maximum against available credits. The run stays within that cap; the service absorbs measured excess, with no automatic overage. An operator's report review settles measured usage once, releases any unused hold, and shows the run and meter breakdown in the customer ledger. Available credits equal the period grant minus settled consumption and active holds. An unfinished run remains tied to the period in which it was authorized, even if review finishes after the next period begins. A queued cancellation before device claim releases the hold. When the balance cannot cover a new maximum, new work is blocked.

Device occupancy is measured with server claim and release timestamps persisted per attempt. Model token counts come from worker completion metadata; only sealed evidence bytes count. Missing meters remain an operational review gate. Historical periods and rate revisions remain in the database; the current customer page shows the active period's balance and recent run usage. Full subscription cancellation self-service, taxes, hosted renewal and real customer-app unit economics remain to be qualified before general availability.
