...
| ---------------------- | ----------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------- |
| `apps/api`             | Rust/Loco API, authorization, domain services, scheduling, verification and reports | Auth, setup, authoring, durable execution, comparison and history projection implemented        |
...
| `apps/web`             | React/Vite dashboard using React Router, TanStack Query, Mantine                    | Authenticated dashboard with setup, saved tests, explicit execution and source-aware Runs       |
| `apps/mobile-worker`   | Python/uv adapter, device observations and evidence                                 | HTTP phone/execution workers, fake adapter and qualified direct boundary implemented            |
...
The UI/API app-setup, authoring, durable execution and evidence flows are implemented.
Hosted S3-compatible storage is configured in code but has no authorized round-trip evidence.
The Python worker claims fenced HTTP jobs, controls the qualified adapter and retains
evidence; synthetic checks do not prove an uploaded customer APK. Tests and Runs expose
durable saved-case/release executions separately from editor trials. Upload validation
readiness must not be presented as execution readiness. Rust owns admission, verification,
outcome aggregation and deterministic baseline comparison.
