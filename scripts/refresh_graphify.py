...
def main() -> None:
    ...
    command = ["uv", "run", "--project", str(ROOT / "tools/graphify"), "--frozen", "graphify"]
    with tempfile.TemporaryDirectory(prefix="mobile-qa-graphify-") as temporary:
        staged = Path(temporary) / "graphify-out"
        subprocess.run(
            [*command, "extract", ".", "--code-only", "--max-workers", "2", "--out", temporary],
            cwd=ROOT, env=env, check=True,
        )
        subprocess.run(
            [*command, "cluster-only", ".", "--graph", str(staged / "graph.json"), "--no-label", "--no-viz"],
            cwd=ROOT, env=env, check=True,
        )
        graph = json.loads((staged / "graph.json").read_text())
        if not graph.get("nodes") or not graph.get("built_at_commit"):
            raise ValueError("Graphify produced an empty graph or omitted its source commit")
        for node in graph["nodes"]:
            source = node.get("source_file")
            if source and (Path(source).is_absolute() or ".." in Path(source).parts):
                raise ValueError(f"Graphify produced a nonportable source path: {source}")
