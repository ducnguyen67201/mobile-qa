...
for command, directory in [
    (
        ["cargo", "fetch"] + (["--locked"] if (ROOT / "Cargo.lock").exists() else []),
        ROOT,
    ),
    (
        ["pnpm", "install", "--ignore-scripts"]
        + (
            ["--frozen-lockfile"] if (ROOT / "apps/web/pnpm-lock.yaml").exists() else []
        ),
        ROOT / "apps/web",
    ),
    (
        ["uv", "sync", "--extra", "sdk"]
        + (["--frozen"] if (ROOT / "apps/mobile-worker/uv.lock").exists() else []),
        ROOT / "apps/mobile-worker",
    ),
]:
